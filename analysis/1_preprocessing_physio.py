...
def qc_eeg(raw, sub, plot_psd=[]):
    ...
    img = ill.image_mosaic([ill.fig2img(i) for i in [fig1, fig2]], nrows=1, ncols=2)
...
def qc_hep(epochs, reject_log, sub, plot_hep=[]):
    ...
    # To image
    plot_hep.append(ill.fig2img(fig))
    plt.close("all")
    return plot_hep
...
def qc_heo(power, itc, sub, plot_heo=[]):
    ...
    plot_heo.append(ill.fig2img(fig))
    plt.close("all")
    return plot_heo
...
for sub in meta["participant_id"].values[0:8:]:
